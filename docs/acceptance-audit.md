...
> 审计日期：2026-05-19。本文把原始目标逐条映射到当前仓库证据，避免把 seeded fixture、mock API 或通过测试误当成真实线上质量/性能结论。

## 1. 交付物映射

| 要求 | 当前证据 | 状态 | 备注 |
|---|---|---:|---|
| 重构后的代码覆盖现代 RAG | `RetrievalService`、`HybridRanker`、`QueryRewriteService`、`ExternalRerankService`、`TextSplitter`、`ChunkStrategy` | 已完成 | 代码已具备 hybrid / rerank / rewrite / parent-child；真实 API runner 已生成 embedding + external rerank 对比 |
| `docs/architecture.md` | `docs/architecture.md` | 已完成 | 包含现代 RAG 流程、缓存、ADR 索引 |
| `docs/evaluation.md` | `docs/evaluation.md`，`RetrievalEvaluationTest`，`DbBackedRetrievalEvaluationIT`，`VersionedRetrievalEvaluationIT`，`RealApiRetrievalEvaluationIT`，`benchmarks/raw-results/real-api-evaluation-report-20260519-035801.md` | 已完成 | seeded matrix 与真实 embedding/external rerank matrix 均有；query rewrite 真实 API 指标仍单独标 TBD |
...
| README 更新 | `README.md` | 部分完成 | 真实质量指标已带前提补充；真实端到端 SSE 性能仍标注待补 |
| `docs/resume-snippet.md` | `docs/resume-snippet.md` | 部分完成 | 已去虚化并补充真实 API 质量口径；真实 SSE 性能仍待跑 |
...
| 二阶段 rerank | `HybridRanker` 本地 rerank；`ExternalRerankService` OpenAI-compatible/DashScope rerank API；`real-api-evaluation-report-20260519-035801.md` | 已完成 |
...
| baseline → +hybrid → +rerank → +query rewrite 表 | `VersionedRetrievalEvaluationIT` 和 `benchmarks/raw-results/versioned-evaluation-report-20260518-232618.md` | seeded 完成 |
| 真实 embedding / rewrite / rerank 版本对比 | `RealApiRetrievalEvaluationIT`、`benchmarks/raw-results/real-api-evaluation-report-20260519-035801.md` | embedding + external rerank 已完成；真实 query rewrite 因本次配置关闭仍 TBD |
...
1. 使用真实 LLM/Embedding/Rerank API 运行 `benchmarks/scripts/run-real-chat-stream-k6.sh`，输出 `/api/v1/chat/stream` P50/P95/P99；mock k6 不能替代真实端到端性能。
2. 如果要在简历写“query rewrite 提升”，必须重新运行启用 `RAG_QUERY_REWRITE_ENABLED=true` 的真实 API 报告；当前真实报告只覆盖 dense / hybrid / local rerank / external rerank。
3. 若要宣称“10 万真实 embedding 语义质量”，还需用真实 embedding 对真实领域文本构建 10 万向量；当前 10 万 real-text fixture 的向量是 deterministic fixture vector，仅适合索引/规模压测。

## 8. Artifact verifier

`node benchmarks/scripts/verify-benchmark-artifacts.mjs --strict` 已验证当前 raw-results 中 real API retrieval matrix 存在，但 real SSE k6 benchmark 仍缺失；`--strict` 模式会在缺失时返回非零，适合最终验收前使用。
